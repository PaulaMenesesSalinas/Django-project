...
def home(request):
    ...
    return render(
        request, 
        "gestionsuppliers.html", {"suppliers": supplierslist})
...
def registerProduct(request):
    if request.method == 'POST':
        ...
        # Obtain the supplier associated with the ID
        ...
        return redirect('registerProduct')  # Redirects back to the product registration page after creating a new product.
...
def editProduct(request, id):
    ...
    supplier_list = suppliers.objects.all()  # Obtain all suppliers.
...
def updateProduct(request, id):
    ...
    if request.method == 'POST':
        ...
        supplier = get_object_or_404(suppliers, id=supplier_id)  # Obtain the object from the supplier.
        product.supplier = supplier  # Assign the supplier to the product.
        product.save()
